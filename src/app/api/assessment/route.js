...
export async function POST(request) {
  try {
    ...
    const { action, courseId, answers = [], eventType } = await request.json();
    if (!courseId || !["start", "submit", "integrity_event"].includes(action)) {
      return NextResponse.json(
        { message: "A valid assessment action is required" },
        { status: 400 },
      );
    }
    ...
    const [user, course] = await Promise.all([
      ...
      Course.findById(courseId).select(
        "+finalAssessment.questions.correctOption +finalAssessment.questions.explanation",
      ),
    ]);
    ...
    if (action === "integrity_event") {
      const allowedEvents = ["back_attempt", "tab_hidden", "fullscreen_exit"];
      if (
      ...
      result.integrityEvents.push({ type: eventType, occurredAt: new Date() });
    ...
    if (action === "start") {
      ...
      if (!active) {
        ...
        result.expiresAt = new Date(
          now.getTime() + course.finalAssessment.durationMinutes * 60_000,
        );
        result.integrityEvents = [];
        await user.save();
      }
    ...
    if (result.status !== "in_progress" || !result.expiresAt) {
      return NextResponse.json(
        { message: "Start the assessment before submitting" },
        { status: 409 },
      );
    }
    if (new Date() > new Date(result.expiresAt.getTime() + 10_000)) {
      ...
      await user.save();
      return NextResponse.json(
        { message: "The assessment time has expired", score: 0, passed: false },
        { status: 408 },
      );
    }

    const submitted = new Map(
      answers.map((answer) => [answer.questionId, Number(answer.option)]),
    );
    const correct = course.finalAssessment.questions.reduce(
      (count, question) =>
        count +
        (submitted.get(question._id.toString()) === question.correctOption
          ? 1
          : 0),
      0,
    );
    const score = Math.round(
      (correct / course.finalAssessment.questions.length) * 100,
    );
    ...
    const certificate = passed
      ? await issueCertificate({ user, course, score })
      : null;
    return NextResponse.json({
