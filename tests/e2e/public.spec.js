...
test("landing page presents the learning catalog", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /give the best start/i })).toBeVisible();
  await expect(page.getByText("Top companies hiring from us")).toBeVisible();
...
test("course search filters the live catalog", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /python/i }).last()).toBeVisible();
  await expect(page.getByText("1 found")).toBeVisible();

  await page.getByPlaceholder("Search by course or category").fill("not-a-real-course");
...
test("course details can be opened directly", async ({ page, request }) => {
  ...
  await expect(page.getByRole("heading", { level: 1, name: `${courses[0].name} Course`, exact: true })).toBeVisible();
